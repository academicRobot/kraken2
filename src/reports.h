...
void ReportMpaStyle(std::string filename, bool report_zeros, const Taxonomy &tax, taxon_counters_t &call_counters);
...
void MpaReportDFS(taxid_t taxid, std::ofstream &ofs, bool report_zeros, const Taxonomy &taxonomy,
...
void ReportKrakenStyle(std::string filename, bool report_zeros, bool report_kmer_data,
    const Taxonomy &taxonomy, taxon_counters_t &call_counters, uint64_t total_seqs, uint64_t total_unclassified);
